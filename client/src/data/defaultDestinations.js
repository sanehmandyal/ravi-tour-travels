export const defaultDestinations = [
  {
    _id: 'dest-1',
    name: 'Dharamshala & McLeodGanj',
    slug: 'dharamshala-mcleodganj',
    state: 'Himachal Pradesh',
    region: 'Kangra Valley',
    tagline: 'The Little Lhasa of India amid cedar woods',
    description: 'Perched in the upper reaches of Kangra Valley, Dharamshala and McLeodGanj are world-renowned for the residence of His Holiness the Dalai Lama, the iconic HPCA International Cricket Stadium, tranquil Buddhist monasteries, Bhagsu waterfall, and scenic Triund trek trails.',
    featuredImage: 'https://images.unsplash.com/photo-1571536802807-30451e3955d8?auto=format&fit=crop&w=1000&q=80',
    images: [
      'https://images.unsplash.com/photo-1571536802807-30451e3955d8?auto=format&fit=crop&w=1000&q=80',
      'https://images.unsplash.com/photo-1596895111956-bf1cf0599ce5?auto=format&fit=crop&w=1000&q=80'
    ],
    bestTimeToVisit: 'March to June & September to December',
    idealDuration: '3 - 4 Days',
    cabFare: 'Custom Hill Quote',
    estimatedBudget: 9999,
    rating: 5.0,
    reviewsCount: 124,
    isFeatured: true,
    attractions: ['Tsuglagkhang Complex', 'HPCA Cricket Stadium', 'Bhagsunag Waterfall & Temple', 'Dal Lake & St. John Church', 'Triund Trek Top'],
    activities: ['Sightseeing Cabs', 'Monastery Walks', 'Cafe Hopping', 'Mountain Photography']
  },
  {
    _id: 'dest-2',
    name: 'Manali & Solang Valley',
    slug: 'manali-solang-valley',
    state: 'Himachal Pradesh',
    region: 'Kullu & Manali',
    tagline: 'Snow-capped peaks, pine forests & thrilling adventure',
    description: 'Nestled on the banks of Beas River, Manali is Himachal’s ultimate holiday hotspot. Marvel at snow-covered slopes at Solang Valley and Rohtang Pass, explore the engineering wonder Atal Tunnel, stroll along Mall Road, and unwind at Hadimba Temple.',
    featuredImage: 'https://images.unsplash.com/photo-1626621341517-bbf3d9990a23?auto=format&fit=crop&w=1000&q=80',
    images: [
      'https://images.unsplash.com/photo-1626621341517-bbf3d9990a23?auto=format&fit=crop&w=1000&q=80',
      'https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=1000&q=80'
    ],
    bestTimeToVisit: 'October to June (Snow: Dec-Feb)',
    idealDuration: '4 - 5 Days',
    cabFare: 'Custom Hill Quote',
    estimatedBudget: 14999,
    rating: 5.0,
    reviewsCount: 156,
    isFeatured: true,
    attractions: ['Solang Valley Snow Point', 'Atal Tunnel Sissu', 'Hadimba Devi Temple', 'Jogini Waterfalls', 'Old Manali Cafes'],
    activities: ['Snow Cab Rides', 'Skiing & Paragliding', 'River Rafting in Beas', 'Rohtang Pass Tours']
  },
  {
    _id: 'dest-3',
    name: 'Dalhousie & Khajjiar',
    slug: 'dalhousie-khajjiar',
    state: 'Himachal Pradesh',
    region: 'Chamba & Dalhousie',
    tagline: 'Mini Switzerland of India with emerald pine meadows',
    description: 'With Victorian-era charm and sprawling emerald meadows, Dalhousie and Khajjiar look right out of a European postcard. Walk around the saucer-shaped Khajjiar Lake encircled by deodar woods, climb Dainkund Peak for 360-degree Himalayan vistas, and enjoy peaceful mountain air.',
    featuredImage: 'https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=1000&q=80',
    images: [
      'https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=1000&q=80'
    ],
    bestTimeToVisit: 'March to November (Snow: Dec-Feb)',
    idealDuration: '3 - 4 Days',
    cabFare: 'Custom Hill Quote',
    estimatedBudget: 11999,
    rating: 5.0,
    reviewsCount: 92,
    isFeatured: true,
    attractions: ['Khajjiar Meadow & Lake', 'Dainkund Peak', 'Kalatop Wildlife Sanctuary', 'Panchpula Springs', 'Subhash Baoli'],
    activities: ['Meadow Horse Riding', 'Chauffeur Hill Sightseeing', 'Zorbing', 'Nature Treks']
  },
  {
    _id: 'dest-4',
    name: 'Bir Billing',
    slug: 'bir-billing',
    state: 'Himachal Pradesh',
    region: 'Kangra Valley',
    tagline: 'World’s 2nd highest paragliding takeoff site',
    description: 'Bir Billing is internationally renowned as the paragliding capital of India. Soar high above Kangra Valley tea gardens and terraced paddy fields, explore ancient Tibetan monasteries at Bir Tibetan Colony, and catch unforgettable sunsets.',
    featuredImage: 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=1000&q=80',
    images: [
      'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=1000&q=80'
    ],
    bestTimeToVisit: 'October to June',
    idealDuration: '2 - 3 Days',
    cabFare: 'Custom Hill Quote',
    estimatedBudget: 7999,
    rating: 5.0,
    reviewsCount: 88,
    isFeatured: true,
    attractions: ['Billing Take-off Site', 'Bir Landing Ground', 'Chokling Monastery', 'Sherab Ling Monastery', 'Palampur Tea Estates'],
    activities: ['Tandem Paragliding', 'Monastery Tours', 'Toy Train Experience', 'Sunset Viewpoints']
  },
  {
    _id: 'dest-5',
    name: 'Shimla & Kufri',
    slug: 'shimla-kufri',
    state: 'Himachal Pradesh',
    region: 'Shimla Hills',
    tagline: 'Queen of the Hills with colonial elegance',
    description: 'The historic summer capital of British India, Shimla captivates with its bustling Mall Road, Neo-Gothic Christ Church, panoramic Ridge, and snow adventures at nearby Kufri.',
    featuredImage: 'https://images.unsplash.com/photo-1589182373726-e4f658ab50f0?auto=format&fit=crop&w=1000&q=80',
    images: [
      'https://images.unsplash.com/photo-1589182373726-e4f658ab50f0?auto=format&fit=crop&w=1000&q=80'
    ],
    bestTimeToVisit: 'All Year Round',
    idealDuration: '3 - 4 Days',
    cabFare: 'Custom Hill Quote',
    estimatedBudget: 12999,
    rating: 5.0,
    reviewsCount: 110,
    isFeatured: true,
    attractions: ['The Ridge & Mall Road', 'Jakhoo Hanuman Temple', 'Kufri Fun World', 'Viceregal Lodge', 'Christ Church'],
    activities: ['Mall Road Walk', 'Kufri Snow Parks', 'Heritage Walks', 'Toy Train Ride']
  },
  {
    _id: 'dest-6',
    name: 'Kangra Fort & Valley Temples',
    slug: 'kangra-fort-temples',
    state: 'Himachal Pradesh',
    region: 'Kangra Valley',
    tagline: 'Ancient royal heritage & sacred Shaktipeeth shrines',
    description: 'Explore the legendary 1,000-year-old Kangra Fort overlooking the Banganga River, followed by sacred darshan at Mata Brajeshwari Devi, Mata Chamunda Devi, and nearby rock-cut Masroor Temples.',
    featuredImage: 'https://images.unsplash.com/photo-1514222134-b57cbb8ce073?auto=format&fit=crop&w=1000&q=80',
    images: [
      'https://images.unsplash.com/photo-1514222134-b57cbb8ce073?auto=format&fit=crop&w=1000&q=80'
    ],
    bestTimeToVisit: 'All Year Round',
    idealDuration: '1 - 2 Days',
    cabFare: 'Custom Hill Quote',
    estimatedBudget: 4999,
    rating: 5.0,
    reviewsCount: 79,
    isFeatured: true,
    attractions: ['Historic Kangra Fort', 'Mata Brajeshwari Devi Temple', 'Mata Chamunda Devi', 'Masroor Rock Cut Temples', 'Maharana Pratap Sagar'],
    activities: ['Temple Darshan Cabs', 'Fort Audio Tour', 'River Photography', 'Local Kangra Cuisine']
  },
  {
    _id: 'dest-7',
    name: 'Spiti Valley & Kaza',
    slug: 'spiti-valley-kaza',
    state: 'Himachal Pradesh',
    region: 'Spiti & Lahaul',
    tagline: 'The Middle Land between India and Tibet',
    description: 'A breathtaking high-altitude desert featuring thousand-year-old cliffside monasteries, the turquoise jewel Chandratal Lake, world’s highest post office at Hikkim, and high mountain passes.',
    featuredImage: 'https://images.unsplash.com/photo-1533473359331-0135ef1b58bf?auto=format&fit=crop&w=1000&q=80',
    images: [
      'https://images.unsplash.com/photo-1533473359331-0135ef1b58bf?auto=format&fit=crop&w=1000&q=80'
    ],
    bestTimeToVisit: 'May to October',
    idealDuration: '6 - 8 Days',
    cabFare: 'Custom Hill Quote',
    estimatedBudget: 24999,
    rating: 5.0,
    reviewsCount: 65,
    isFeatured: true,
    attractions: ['Key Monastery', 'Chandratal Lake', 'Hikkim Highest Post Office', 'Komic Village', 'Pin Valley National Park'],
    activities: ['4x4 Off-Road Cab Expeditions', 'Stargazing', 'Fossil Hunting in Langza', 'Monastery Stays']
  },
  {
    _id: 'dest-8',
    name: 'Kasol & Parvati Valley',
    slug: 'kasol-parvati-valley',
    state: 'Himachal Pradesh',
    region: 'Kullu & Manali',
    tagline: 'Scenic riverside hamlets & hot springs',
    description: 'Flanked by the raging Parvati River and dense coniferous forests, Kasol is celebrated for its laid-back cafes, hot sulfur springs of holy Manikaran Sahib, and scenic trail walks.',
    featuredImage: 'https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=1000&q=80',
    images: [
      'https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&w=1000&q=80'
    ],
    bestTimeToVisit: 'March to June & September to November',
    idealDuration: '2 - 3 Days',
    cabFare: 'Custom Hill Quote',
    estimatedBudget: 6999,
    rating: 5.0,
    reviewsCount: 72,
    isFeatured: false,
    attractions: ['Manikaran Gurudwara & Hot Springs', 'Chalal Village Trek', 'Tosh Village', 'Parvati River Banks'],
    activities: ['Riverside Camping', 'Hot Spring Baths', 'Cafe Exploration', 'Trekking']
  },
  {
    _id: 'dest-9',
    name: 'Jibhi & Tirthan Valley',
    slug: 'jibhi-tirthan-valley',
    state: 'Himachal Pradesh',
    region: 'Kullu & Manali',
    tagline: 'Hidden paradise of waterfalls & cedar forests',
    description: 'An untouched haven of wooden houses, whispering pine forests, secret waterfalls, and Jalori Pass viewpoints. Gateway to the UNESCO World Heritage Great Himalayan National Park.',
    featuredImage: 'https://images.unsplash.com/photo-1596895111956-bf1cf0599ce5?auto=format&fit=crop&w=1000&q=80',
    images: [
      'https://images.unsplash.com/photo-1596895111956-bf1cf0599ce5?auto=format&fit=crop&w=1000&q=80'
    ],
    bestTimeToVisit: 'March to November',
    idealDuration: '3 - 4 Days',
    cabFare: 'Custom Hill Quote',
    estimatedBudget: 8999,
    rating: 5.0,
    reviewsCount: 54,
    isFeatured: false,
    attractions: ['Jibhi Waterfall', 'Jalori Pass & Serolsar Lake', 'Chehni Kothi Fort', 'Tirthan River Trout Fishing'],
    activities: ['Nature Walk & Forest Hikes', 'Trout Angling', 'Photography', 'Jalori Pass Cab Drive']
  },
  {
    _id: 'dest-10',
    name: 'Jawalamukhi & Chintpurni',
    slug: 'jawalamukhi-chintpurni',
    state: 'Himachal Pradesh',
    region: 'Kangra Valley',
    tagline: 'The eternal divine flame & wish-fulfilling shrines',
    description: 'One of the most sacred Shaktipeeth shrines in India, Jawalamukhi Temple is renowned for its eternal natural blue flames burning continuously without any fuel. Paired with Chintpurni Devi and Baglamukhi.',
    featuredImage: 'https://images.unsplash.com/photo-1514222134-b57cbb8ce073?auto=format&fit=crop&w=1000&q=80',
    images: [
      'https://images.unsplash.com/photo-1514222134-b57cbb8ce073?auto=format&fit=crop&w=1000&q=80'
    ],
    bestTimeToVisit: 'All Year Round',
    idealDuration: '2 Days',
    cabFare: 'Custom Hill Quote',
    estimatedBudget: 5999,
    rating: 5.0,
    reviewsCount: 80,
    isFeatured: false,
    attractions: ['Mata Jawalamukhi Temple (Eternal Flame)', 'Mata Chintpurni Devi Shrine', 'Mata Baglamukhi Temple', 'Beas River Viewpoint'],
    activities: ['Divine Darshan', 'Aarti Ceremonies', 'Pilgrimage Cab Package', 'Kangra Sightseeing']
  }
].map(d => ({
  ...d,
  heroImage: d.heroImage || d.featuredImage,
  featuredImage: d.featuredImage || d.heroImage
}));
